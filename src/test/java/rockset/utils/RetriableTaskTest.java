...
public class RetriableTaskTest {
  ...
  public void testRetriableTask() throws Exception {
    ...
    RetriableTask retriableTask = new RetriableTask(
        blockingExecutor, retryExecutorService, runnable);
    ...
    }).when(blockingExecutor).submit(retriableTask);
  ...
  public void testRetryRejected() throws Exception {
    ...
    Mockito.doThrow(RejectedExecutionException.class)
        .when(retryExecutorService).submit(Mockito.any(Runnable.class));
